...
res = cur.execute(
    ...
    WHERE compound_id NOT IN (SELECT compound_id FROM runs WHERE run_type == 'FrozenBarCENSO3E') \
    AND compound_id IN (SELECT compound_id FROM runs WHERE run_type == 'FrozenForwardCENSO3E' AND status = 'Received') \
    AND compound_id IN (SELECT compound_id FROM runs WHERE run_type == 'FrozenReversedCENSO3E' AND status = 'Received') \
...
res = cur.execute(
    ...
    AND run_type = 'FrozenForwardCENSO3E'"
)
...
res = cur.execute(
    f"SELECT local_path FROM runs \
    WHERE compound_id = '{compound_id}' \
    AND run_type = 'FrozenReversedCENSO3E'"
)
...
cur.execute(
    f"INSERT INTO runs VALUES ('{compound_id}', 'FrozenBarCENSO3E', 'Received', {local_path}, 'localhost', '/data/michael/misc/17-rir/frozen-solute-model/data/{local_path}/');"
)
con.commit()
con.close()
