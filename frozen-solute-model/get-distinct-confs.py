...
for compound_id, smiles, iupac, experimental, _, mobley, _, _, _, _, _, _ in molecules:
    ...
    censo_conformer = (
        subprocess.run(
            ["grep", "CONF", f"data/{censo_path}/2_OPTIMIZATION.xyz"],
            check=True,
            capture_output=True,
        )
        .stdout.decode("utf-8")
    ...
    vacuum_censo_conformer = (
        subprocess.run(
            ["grep", "CONF", f"data/{vacuum_censo_path}/2_OPTIMIZATION.xyz"],
            check=True,
            capture_output=True,
        )
        .stdout.decode("utf-8")
    ...
    if censo_conformer != vacuum_censo_conformer:
        ...
        subprocess.run(
            [
                "cp",
                f"data/{censo_path}/2_OPTIMIZATION.xyz",
                f"verify/{compound_id}/censo_input.xyz",
            ],
            check=True,
        )

        subprocess.run(
            ["python", "trim_xyz.py", f"verify/{compound_id}/censo_input.xyz"],
            check=True,
        )
        ...
        subprocess.run(
            [
                "cp",
                f"data/{vacuum_censo_path}/2_OPTIMIZATION.xyz",
                f"verify/{compound_id}/vacuum_censo_input.xyz",
            ],
            check=True,
        )

        subprocess.run(
            ["python", "trim_xyz.py", f"verify/{compound_id}/vacuum_censo_input.xyz"],
            check=True,
        )
        ...
        with open(f"verify/{compound_id}/censo.inp", "w") as f:
            f.write(
                orca_inp_template.format(
                    basis=basis, solvation="SMD(WATER)", input="censo_input.xyz"
                )
            )
        ...
        with open(f"verify/{compound_id}/vacuum_censo.inp", "w") as f:
            f.write(
                orca_inp_template.format(
                    basis=basis, solvation="", input="vacuum_censo_input.xyz"
                )
            )
